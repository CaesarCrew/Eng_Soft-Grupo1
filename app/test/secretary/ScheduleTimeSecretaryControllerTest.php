...
class ScheduleTimeSecretaryControllerTest extends TestCase
{
    ...
    private function postSelectTime(array $postData)
    {
        ...
        $ch = curl_init('http://localhost/selecionarHorario');
    ...
    public function testSelectTime()
    {
        ...
        $postData = [
            'selected_schedules' => [$selectedSchedules[0]],
            'secretary_id' => '1',
            'cpf' => '24761283092'
        ];
    ...
    public function testSelectTimeWithInvalidData()
    {
        ...
        $response = $this->postSelectTime($postData);
        $statusCode = curl_getinfo($response['ch'], CURLINFO_HTTP_CODE);
        $this->assertEquals(400, $statusCode, "O status code esperado era 400 para dados inválidos");
    ...
    public function testSelectTimeWithNonExistentSchedule()
    {
        $postData = [
            'selected_schedules' => [999999],
            'secretary_id' => '1',
            'cpf' => '24761283092'
        ];
        $response = $this->postSelectTime($postData);
        $statusCode = curl_getinfo($response['ch'], CURLINFO_HTTP_CODE);
        $this->assertEquals(500, $statusCode, "O status code esperado era 404 para seleção de horário inexistente");
